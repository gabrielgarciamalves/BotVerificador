...
def carregar_historico():
    ...
    padrao = {"STN": [], "TCE-SP": [], "TCE-MG": [], "TCE-RJ": []}
...
def verificar_tce_rj(historico, novidades):
    """Verifica atualizações no TCE-RJ extraindo apenas a linha exata e limitando o tamanho para o Discord."""
    url = "https://www.tcerj.tc.br/portalnovo/pagina/sigfis_municipal_versao_atualizada"
    print("Verificando TCE-RJ...")
    
    try:
        import urllib3
        # Oculta os alertas de certificado SSL inválido/expirado (comum em sites do governo)
        urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
        ...
        # O Ctrl+F focando diretamente no ano
        nos_texto = soup.find_all(string=re.compile(r'202[6-9]'))
        
        documentos_recentes = {}
        
        for node in nos_texto:
            # Sobe no máximo 2 níveis para pegar a linha sem pegar o painel inteiro
            bloco = node.parent
            if bloco.name not in ['li', 'p', 'td', 'div', 'a'] and bloco.parent:
                bloco = bloco.parent
                
            texto_completo = bloco.get_text(" ", strip=True)
            
            # Trava de segurança contra blocos gigantes
            if len(texto_completo) > 200:
                texto_completo = node.get_text(" ", strip=True)
                
            texto_lower = texto_completo.lower()
            
            # Filtro para ignorar textos soltos que não sejam os documentos alvo
            termos_validos = ["manual", "sigfis", "versão", "versao", "atualizado", "módulo", "modulo"]
            if not any(termo in texto_lower for termo in termos_validos):
                continue
                
            anos_encontrados = re.findall(r'\b(202[6-9])\b', texto_lower)
            if not anos_encontrados:
                continue
                
            maior_ano = max([int(a) for a in anos_encontrados])
            
            # Busca data de atualização
            match_data = re.search(r'(\d{2}[\./-]\d{2}[\./-]\d{4})', texto_lower)
            data_doc = match_data.group(1) if match_data else "Data não identificada"
            
            # Tenta caçar o link de download dentro daquela pequena linha
            link_tag = bloco.find('a', href=True)
            if not link_tag and hasattr(node.parent, 'find'):
                link_tag = node.parent.find('a', href=True)
            ...
            if href.startswith('#') or 'javascript' in href:
                ...
                href = "https://www.tcerj.tc.br" + href
            elif not href.startswith('http'):
                href = f"https://www.tcerj.tc.br/portalnovo/pagina/{href}"
            
            # Limpeza e limitação do nome para não estourar o limite do Discord
            nome_exibicao = re.sub(r'\s+', ' ', texto_completo).strip()
            if len(nome_exibicao) > 150:
                nome_exibicao = nome_exibicao[:147] + "..."
                
            info_completa = f"Versão {maior_ano} | Atualizado em: {data_doc}"
            identificador_historico = f"{nome_exibicao} | {info_completa}"
            
            if identificador_historico not in documentos_recentes:
                documentos_recentes[identificador_historico] = {
                    "documento": nome_exibicao,
                    "info": info_completa,
                    "link": href
                }
                
        # Gravação e disparo para o Discord
        for idx, item in documentos_recentes.items():
            if idx not in historico["TCE-RJ"]:
                novidades["TCE-RJ"].append(item)
                historico["TCE-RJ"].append(idx)
                print(f"Novo documento TCE-RJ notificado: {item['documento']} ({item['info']})")
                
    except Exception as e:
        print(f"Erro ao verificar TCE-RJ: {e}")

if __name__ == "__main__":
    historico_atual = carregar_historico()
    
    novidades_atuais = {"STN": [], "TCE-SP": [], "TCE-MG": [], "TCE-RJ": []}
    ...
    verificar_tce_rj(historico_atual, novidades_atuais)
